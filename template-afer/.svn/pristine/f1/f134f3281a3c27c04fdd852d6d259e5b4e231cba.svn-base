...
class MeekroDB {
 ...
 /*  if ($result) {
   if (is_array($result)) {array_walk_recursive($result, function (&$value) {$value = htmlentities($value,null,null,false);});}
   else if (is_object($result)) {}
   else {$result = htmlentities($result,null,null,false);}
   }
  ...
  public function fetchRow($result = null) {
    ...
    if ($row) array_walk_recursive($row, function (&$value) {$value = htmlspecialchars($value,ENT_NOQUOTES);});
    return $row;
  }
  ...
  public function fetchArray($result = null) {
    ...
    //    if ($row) array_walk_recursive($row, function (&$value) {$value = str_replace(array("<",">"),"",$value);});
